...
def get_student_record_book_marks_detailed(
        db: Session,
        student_external_id: str,
        student_class: str = "11-Т"
) -> RecordBookResponse:
    ...
    for event in accessible_events:
        # Получаем все этапы для этого типа мероприятия с возможными результатами
        stages_with_results = (
            db.query(Stage, PossibleResult)
            .join(PossibleResult, Stage.id == PossibleResult.stage_id)
            .filter(Stage.event_type_id == event.event_type_id)
            .all()
        )
        ...
        # Группируем этапы
        stages_dict = {}
        for stage, possible_result in stages_with_results:
            if stage.id not in stages_dict:
                stages_dict[stage.id] = {
                    'stage': stage,
                    'possible_results': []
                }
            stages_dict[stage.id]['possible_results'].append(possible_result)
        ...
        for stage_id, stage_data in stages_dict.items():
            ...
            # Ищем достижение студента для этого этапа
            achievement = (
                db.query(Achievement)
                .join(PossibleResult, Achievement.result_id == PossibleResult.id)
                .filter(
        ...
        event_status = "зачет" if all_stages_completed and stages_dict else "незачет"
...
# Версия с агрегацией по event_type (если нужно группировать по типам мероприятий)
def get_student_record_book_by_event_type(
        db: Session,
        student_external_id: str,
        student_class: str = "11-Т"
) -> RecordBookResponse:
    # Получаем доступные типы мероприятий для класса
    accessible_event_types = (
        db.query(EventType)
    ...
    for event_type in accessible_event_types:
        # Получаем все мероприятия этого типа
        events = (
            db.query(Event)
            .filter(Event.event_type_id == event_type.id)
            .filter(Event.is_active == True)
            .all()
        )
        ...
        for event in events:
            stages = (
                db.query(Stage)
                .filter(Stage.event_type_id == event_type.id)
                .all()
            )
